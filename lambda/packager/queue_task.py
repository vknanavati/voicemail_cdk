...
TASK_TITLE = "New Voicemail Message"
...
def build_task_attributes(payload: dict) -> dict:
    ...
    attributes = {
        'voicemail_queue': payload.get('voicemail_queue_arn', ''),
        "DestinationType": payload.get('destination_type', 'Queue'),
...
def connect_queue_task(payload: dict) -> dict:
    """Create Connect task for voicemail notification"""
    try:
        logger.info("Creating queue task for voicemail")
        ...
        task_params = {
            ...
            "Description": DESCRIPTION_TEMPLATE.format(
                payload.get('vmQueueName', 'Queue Assignment')
            ),
        ...
        # Create task in Amazon Connect (NO PreviousContactId - let Connect assign)
